...
📌 Цель:
Реализовать систему лототрона, которая хранит список участников и случайным образом выбирает их без повторов, пока список не закончится.
...
public void init()
Инициализирует лототрон: перемешивает список и добавляет участников в очередь.   (Collections.shuffle - этот метод перемешивает колекцию которую отправляете в аргументе)
